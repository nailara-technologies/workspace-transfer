...
say "🔔 QUESTION-ANSWER OUT-OF-BAND COMMUNICATION SYSTEM";
say "=" x 80;
say "";
...
unless (-f $questions_answered_log) {
    ...
    print $fh "# Auto-generated by question-answer-system.pl\n\n";
    close $fh;
    say "✅ Created $questions_answered_log";
}
...
sub check_question_files {
    ...
    say "📬 Found " . scalar(@question_files) . " question file(s)";
    say "-" x 80;
...
sub check_answer_files {
    ...
    say "📬 Found " . scalar(@answer_files) . " answer file(s)";
    say "-" x 80;
    ...
    for my $afile (@answer_files) {
        ...
        if ($matched) {
            ...
            say "   ✅ Matched with pending question";
        } else {
            say "   ⚠️  No pending question found (standalone answer)";
        }
    ...
    if (@pending) {
        ...
        for my $line (@pending) {
            if ($line =~ /^\[([^\]]+)\] PENDING \| ([^|]+) \| (?:PRIORITY:(\w+) \| )?(.+)$/) {
                ...
                my $prefix = $priority eq 'URGENT' ? '🔴' : '🟡';
                say "$prefix [$timestamp] → $target";
                say "   $question";
                say "";
            }
        }
    } else {
        say "✅ No pending questions";
    }
}
...
EOF
    ...
    say "✅ Created git post-commit hook: $hook_file";
}
...
EOF
    ...
    say "✅ Created documentation: $doc_file";
}
...
if ($action eq '--check') {
    ...
    say "🔧 SETTING UP QUESTION-ANSWER SYSTEM";
    say "-" x 80;
    say "";
    create_git_hook();
    ...
    say "  Git hook: " . (-f "$repo_root/.git/hooks/post-commit" ? "✅ Installed" : "❌ Not installed");
    say "";
    
    if (@pending) {
        say "⚠️  Action needed: " . scalar(@pending) . " pending question(s)";
    } else {
        say "✅ No pending questions";
    }
    say "=" x 80;

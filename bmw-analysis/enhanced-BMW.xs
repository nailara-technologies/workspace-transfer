...
getstate (self)
    Digest::BMW self
CODE:
    RETVAL = newSVpvn((char *)self, sizeof(hashState));
OUTPUT:
    RETVAL

void
setstate (self, state_sv)
    Digest::BMW self
    SV* state_sv
PREINIT:
    STRLEN len;
    char *state_data;
PPCODE:
    state_data = SvPV(state_sv, len);
    if (len != sizeof(hashState)) {
        croak("Invalid BMW state size: got %lu, expected %lu", 
              (unsigned long)len, (unsigned long)sizeof(hashState));
    }
    Copy(state_data, self, 1, hashState);
    XSRETURN(1);
